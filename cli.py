#!/usr/bin/env python3
"""
Punkito Tabs Oracle - CLI Orchestrator
Módulo central encargado de la interfaz de línea de comandos, validación del sistema
operativo (ffmpeg), carga dinámica de configuraciones de idioma (i18n) y control del pipeline.
"""

import os
import sys
import argparse
import json
import shutil
from pathlib import Path
from typing import Dict, Any

def resolver_ruta_config(lang: str) -> Path:
    """
    Resuelve dinámicamente la ruta física del archivo JSON de traducción.
    Soporta ejecución en modo editable (pip install -e .) y estructura empaquetada.
    """
    # Ubicación de este script: src/punkito_tabs_oracle/cli.py
    directorio_actual = Path(__file__).resolve()
    
    # Intento 1: Buscar en el directorio raíz del repositorio (modo desarrollo local)
    # parent[0] = punkito_tabs_oracle, parent[1] = src, parent[2] = raíz del proyecto
    ruta_desarrollo = directorio_actual.parents[2] / "config" / "locales" / f"{lang}.json"
    if ruta_desarrollo.exists():
        return ruta_desarrollo
        
    # Intento 2: Buscar relativo al paquete (por si se distribuye como sub-paquete en el futuro)
    ruta_paquete = directorio_actual.parent / "config" / "locales" / f"{lang}.json"
    if ruta_paquete.exists():
        return ruta_paquete
        
    # Si no se encuentra en ningún lado, arrojamos excepción de diseño de sistema
    raise FileNotFoundError(
        f"[-] Error crítico de arquitectura: No se encontró el diccionario locales para '{lang}'."
    )

def cargar_locales(lang: str) -> Dict[str, str]:
    """Carga y decodifica el diccionario JSON correspondiente al idioma de ejecución."""
    try:
        ruta_archivo = resolver_ruta_config(lang)
        with open(ruta_archivo, "r", encoding="utf-8") as f:
            return json.load(f)
    except Exception as e:
        # Fallback de seguridad en consola si falla el parseo de i18n
        print(f"[-] Critical i18n loading failure for language '{lang}': {e}", file=sys.stderr)
        sys.exit(1)

def verificar_ffmpeg() -> bool:
    """
    Verifica si ffmpeg está instalado y accesible en las variables de entorno (PATH).
    Spleeter requiere ffmpeg obligatoriamente para decodificar flujos polifónicos de entrada.
    """
    return shutil.which("ffmpeg") is not None

def validar_audio_entrada(ruta_archivo: str, locales: Dict[str, str]) -> Path:
    """
    Valida la existencia del archivo de audio y su extensión.
    Filtra los formatos compatibles con el backend de Librosa/Soundfile.
    """
    path_obj = Path(ruta_archivo).resolve()
    
    if not path_obj.exists():
        print(locales["ERROR_FILE_NOT_FOUND"].format(ruta_archivo), file=sys.stderr)
        sys.exit(1)
        
    extensiones_validas = {".mp3", ".wav", ".flac", ".m4a", ".ogg"}
    if path_obj.suffix.lower() not in extensiones_validas:
        print(locales["ERROR_INVALID_EXTENSION"].format(path_obj.suffix, ", ".join(extensiones_validas)), file=sys.stderr)
        sys.exit(1)
        
    return path_obj

def ejecutar_pipeline(audio_path: Path, locales: Dict[str, str]):
    """
    Orquesta la ejecución secuencial de los motores cargándolos bajo demanda.
    Las importaciones se realizan localmente para optimizar el tiempo de carga de la CLI
    evitando inicializar TensorFlow/Librosa innecesariamente si la validación falla.
    """
    print(locales["STATUS_LOAD_MODEL"])
    try:
        # Importación tardía (Lazy Loading) para evitar retrasos en el Parser de Argumentos
        from punkito_tabs_oracle.ml.separator import BassSeparator
        # En la siguiente fase aquí instanciamos el separador
        pass
    except ImportError:
        print("[-] Warning: ml/separator.py is currently empty (stub mode). Skipping ML layer.")

    print(locales["STATUS_DSP_START"])
    try:
        from punkito_tabs_oracle.dsp.pitch import PitchTracker
        # En la fase 3 aquí calculamos pYIN
        pass
    except ImportError:
        print("[-] Warning: dsp/pitch.py is currently empty (stub mode). Skipping DSP layer.")

    print(locales["STATUS_TAB_GEN"])
    try:
        from punkito_tabs_oracle.tab.router import FretboardRouter
        # En la fase 4 aquí resolvemos el camino mínimo
        pass
    except ImportError:
        print("[-] Warning: tab/router.py is currently empty (stub mode). Skipping Router layer.")

    print(locales["SUCCESS_PIPELINE"])

def main():
    # Inicialización del parser básico para evitar cargas pesadas antes de procesar opciones básicas
    parser = argparse.ArgumentParser(
        description="Punkito Tabs Oracle - Structural CLI & Audio Pipeline Orchestrator"
    )
    parser.add_argument(
        "audio_file", 
        type=str, 
        nargs="?", 
        help="Path absolute or relative to the source audio file"
    )
    parser.add_argument(
        "--lang", 
        type=str, 
        choices=["en", "es"], 
        default="en", 
        help="Interface and log execution language"
    )
    args = parser.parse_args()

    # 1. Carga inicial del motor de strings localizado
    locales = cargar_locales(args.lang)

    # 2. Requerimiento posicional del archivo de audio
    if not args.audio_file:
        print(locales["ERROR_NO_AUDIO_PROVIDED"], file=sys.stderr)
        parser.print_help()
        sys.exit(1)

    print(locales["STATUS_VERIFYING_DEPS"])

    # 3. Validar dependencias nativas del host de ejecución (ffmpeg)
    if not verificar_ffmpeg():
        print(locales["ERROR_FFMPEG_MISSING"], file=sys.stderr)
        sys.exit(1)

    # 4. Validar el contenedor de audio
    audio_path_validado = validar_audio_entrada(args.audio_file, locales)

    # 5. Lanzar el motor de inferencia y DSP
    ejecutar_pipeline(audio_path_validado, locales)

if __name__ == "__main__":
    main()